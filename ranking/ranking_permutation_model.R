...
#### Jump and replacement model ####
# with N0 is constant over time
# perm prob is jump proba here
perm_proba <- seq(0.05, 1, 0.05)
...
for (p in 1:length(perm_proba)) {
  ...
  for (t in 2:length(timest)) {
    permutation_proba[[t]] <- jump_replacement_not_full_replace(vector = permutation_proba[[t-1]], prob = perm_proba[p])
  }
...
model_output <- rbindlist(model_list_turnover) %>%
  as_tibble(x = .) 

# model output
write.csv(x = model_output,
          file = "outputs_data/models/o_t_jumpmodel_equals_size1000_t10_p0_to_0_5.csv", 
          row.names = FALSE)
...
jumpdiffmodel %>%
  ...
  ggplot(aes(x = time_t/10, y = Ot, color = N0/N, group = N0/N)) +
  scale_color_viridis_c(option = "C") +
  geom_line() +
  theme_bw() +
  scale_x_continuous(name = TeX(r"($t/T$)")) +
  scale_y_continuous(name = TeX(r"($O_{t}$)")) +
...
### Ft
jumpdiffmodelft <- read.csv(file = "outputs_data/f_t_jumpdiffmodel_size1000_t10_p11_v2.csv") %>%
  as_tibble()

jumpdiffmodelft %>%
  filter(N0 %in% seq(1,1000,20)) %>%
  mutate(`N0/N` = N0/1000) %>%
  ggplot(aes(x = time_t1/10, y = Ft/1000, group = `N0/N`, color = `N0/N`)) +
  scale_color_viridis_c(option = "C") +
  geom_line() +
  theme_bw() +
  scale_x_continuous(name = TeX(r"($t/T$)")) +
  scale_y_continuous(name = TeX(r"($F_{t}$)"), breaks = seq(0,1,0.25), limits = c(0,1)) +
  labs(caption = "J. Gravier, 2022", 
       title = "Rank flux", subtitle = "N=1000, t=10, facetting is P(P)") +
  facet_wrap(~round(proba,3))

ggsave(filename = "rank_flux_2nd_models.png", plot = last_plot(), width = 25, height = 18, units = 'cm')

# rqnk flux mean
jumpdiffmodelft <- read.csv(file = "outputs_data/Flux_mean_t_jumpdiffmodel_size1000_t10_p11_v2.csv") %>%
  as_tibble()

jumpdiffmodelft %>%
  ggplot(aes(x = N0/1000, y = Ft_proba, color = proba, group = factor(proba))) +
  scale_color_viridis_c() +
  geom_line() +
  theme_bw() +
  scale_x_continuous(name = TeX(r"($N_{0}/N$)")) +
  ylab(TeX(r"($F$)")) +
  labs(caption = "J. Gravier, 2022", 
       title = "Mean rank flux rate, N=1000, t=10")

ggsave(filename = "rank_flux_rate_nornalized_2nd_models.png", 
       plot = last_plot(), width = 18, height = 12, units = 'cm')
