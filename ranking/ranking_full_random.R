...
for (t in 2:length(t)) {
  ...
  permutation_proba[[t]] <- jump_permutation(vector = permutation_proba[[t-1]], prob = 0.25)
...
# output data calculations
write.csv(x = tableau_turnover, file = "o_t_jumpmodel_size1000_t10.csv", 
          row.names = FALSE)
...
for (t in 2:length(t)) {
  ...
  permutation_proba[[t]] <- jump_diff_permutation(vector = permutation_proba[[t-1]], 
                                                  prob = 0.25*length(permutation_proba[[t-1]]))
...
write.csv(x = tableau_turnover, file = "o_t_jumpdiffmodel_size1000_t10.csv", 
          row.names = FALSE)

#### plotting diverse models ####
tableau_turnover <- read.csv(file = "o_t_jumpmodel_size1000_t10.csv") %>%
  as_tibble()

tableau_turnover_m1 <- read.csv(file = "o_t_jumpdiffmodel_size1000_t10.csv") %>%
  as_tibble()

tableau_turnover <- tableau_turnover %>%
  mutate(model = "Permutation only") %>%
  bind_rows(tableau_turnover_m1 %>% 
              mutate(model = "Permutation & diffusion"))
